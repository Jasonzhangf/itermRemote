...
- (void)startWithWindowId:(uint32_t)windowId windowNameFallback:(NSString *)windowName fps:(NSInteger)fps cropRectNormalized:(NSDictionary *)cropRect {
  ...
  [SCShareableContent getShareableContentWithCompletionHandler:^(SCShareableContent * _Nullable content,
    ...
    if (error || !content) {
      if (error) {
        NSLog(@"[SCK] getShareableContent failed: domain=%@ code=%ld desc=%@", error.domain, (long)error.code, error.localizedDescription);
      } else {
        NSLog(@"[SCK] getShareableContent failed: no content");
      }
      return;
    }
    ...
    NSLog(@"[SCK] Selected window: title=%@ id=%u", target.title, target.windowID);
  ...
  */
  ...
  BOOL useDefaultScreen = NO;
    ...
    if (deviceId != nil && [deviceId isKindOfClass:[NSDictionary class]]) {
      ...
      id winIdAny = mandatory[@"windowId"];
      if (winIdAny != nil && [winIdAny isKindOfClass:[NSNumber class]]) {
        directWindowId = (NSNumber*)winIdAny;
        NSLog(@"[SCK-direct] parsed mandatory.windowId=%@", directWindowId);
      }
  ...
  if (useDefaultScreen) {
    ...
    // New path: direct ScreenCaptureKit window capture via native CGWindowID.
    // This bypasses DesktopCapturer sourceId mapping which is unreliable on newer macOS.
    if (directWindowId != nil && @available(macOS 12.3, *)) {
      ...
      uint32_t resolvedWindowId = [directWindowId unsignedIntValue];
      [sckCapturer startWithWindowId:resolvedWindowId windowNameFallback:@"" fps:fps cropRectNormalized:cropRect];
      NSLog(@"start desktop capture (SCK-direct): fps: %lu, windowId: %u", fps, resolvedWindowId);
      self.videoCapturerStopHandlers[trackUUID] = ^(CompletionHandler handler) {
        NSLog(@"stop desktop capture (SCK-direct): windowId: %u, trackID %@", resolvedWindowId, trackUUID);
        [sckCapturer stop];
        handler();
      };
      desktopCapturer = nil;
      ...
      return;
	    ...
	    if (source.sourceType == RTCDesktopSourceTypeWindow) {
          ...
          sckCapturer.sourceId = sourceId ?: @"";
	        ...
	        // IMPORTANT: `sourceId` is not guaranteed to be the native windowId.
	        // Prefer the metadata we derived from ScreenCaptureKit.
	        uint32_t resolvedWindowId = 0;
	        if (sourceId) {
	          NSDictionary* meta = flutter_sck_lookup_window_meta(sourceId);
	          if (meta) {
	            NSNumber* wid = meta[@"windowId"];
	            if (wid) {
	              resolvedWindowId = [wid unsignedIntValue];
	            }
	          }
	        }
	        if (resolvedWindowId == 0) {
	          resolvedWindowId = (uint32_t)[sourceId intValue];
	        }
	        [sckCapturer startWithWindowId:resolvedWindowId windowNameFallback:source.name fps:fps cropRectNormalized:cropRect];
	        NSLog(@"start desktop capture (SCK): sourceId: %@, type: window, fps: %lu, windowId: %u", sourceId, fps, resolvedWindowId);
        self.videoCapturerStopHandlers[trackUUID] = ^(CompletionHandler handler) {
