...
class WebRTCBlock implements Block {
  ...
  Future<Ack> handle(Command cmd) async {
    try {
      ...
      // ignore: avoid_print
      print('[WebRTCBlock] ERROR in handle: $e');
      // ignore: avoid_print
      print('[WebRTCBlock] STACK: $st');
      return Ack.fail(
        id: cmd.id,
        code: 'webrtc_error',
        message: '$e\n$st',
  ...
  Future<Ack> _startLoopback(Command cmd) async {
    ...
    final sourceType = sourceTypeAny is String && sourceTypeAny.isNotEmpty ? sourceTypeAny : 'screen';

    final sourceIdAny = payload['sourceId'];
    final sourceId = sourceIdAny?.toString();

    final cropRect = payload['cropRect'];
    ...
    final bitrateKbps = bitrateAny is int
        ? bitrateAny
        : int.tryParse('${bitrateAny ?? ''}') ??
            computeHighQualityBitrateKbps(width: width, height: height);

    // Build constraints exactly like cloudplayplus_stone format
    ...
    if (cropRect != null) {
      videoConstraints['cropRect'] = cropRect;
    }
    ...
    // ignore: avoid_print
    print('[WebRTCBlock] mediaConstraints=$mediaConstraints');
  ...
  Future<Ack> _createOffer(Command cmd) async {
    ...
    var sdp = offer.sdp ?? '';
    sdp = sdp.replaceAll('profile-level-id=640c33', 'profile-level-id=42e032');
    sdp = sdp.replaceAll('profile-level-id=640c1f', 'profile-level-id=42e032');
    sdp = sdp.replaceAll('profile-level-id=42e033', 'profile-level-id=42e032');

    final fixedSdp = _fixSdpBitrate(sdp, bitrate);
