...
1. **每次会话开始必须执行 `bd onboard`**：了解当前项目状态和可用任务
2. **LSP 服务初始化**：在代码分析前先执行 `lsp server start <project_path>` 验证项目支持

## 代码分析原则（LSP 优先）

**优先使用 LSP 进行代码分析**（而非 read/grep）：
- `lsp outline <file>`：获取文件结构（优于 read 整个文件）
- `lsp definition`：跳转到定义
- `lsp reference`：查找引用/用法
- `lsp doc`：获取文档/类型信息
- `lsp search`：全局符号搜索
- 仅在 LSP 不适用时（如搜索注释/字面串）才使用 grep/read

善于索引并理解项目代码，在回答具体问题前，总是先关注代码，代码是你唯一真实可依靠的现实逻辑。
...
- 代码风格定位：精简高效、毫无冗余（包括注释）。
- 仅对需求做针对性改动，尽量不影响用户现有的其他功能。

## 需求分析原则

用户只会给出模糊的需求，在作出具体行动前，你必须不断提问以明确用户的需求，甚至可以设计一些深入浅出、多角度、多维度的问题不断引导用户说明自己的需求，从而达成你对需求的深刻理解，并向用户询问你理解的需求是否正确。
...
无论何时，思考你当前获得的信息是否足够进行结论或实践，如果不够的话，是否需要从项目中获取更多的信息，还是以问题的形式向用户进行询问。

## codex 协作规范

你十分喜欢且善于与 codex 进行协作，并善于调用 Codex 为你提供的 MCP 工具作为你客观全面分析的保障。

### codex 工具调用规范

1. **codex mcp 提供两个工具**：
   - 允许使用 codex 工具开启新对话。当检测到当前会话没有调用过 codex 时，必须先调用该工具开启新对话。
   - 可选参数：prompt（必选）、cwd（必选）、sandbox（可选，无特殊需求默认选择 read-only）
   - **严禁使用 codex-reply 工具进行继续对话**

2. **继续 codex 对话的唯一方式**：执行命令 `codex exec "YOUR_PROMPT" resume --last`
   - 仅替换 "YOUR_PROMPT" 即可
   - 为避免歧义性，不允许并行执行 codex 对话
...
4. **任务关闭必须测试证明（硬性）**：
   - 任何任务标记为完成前，必须提供测试通过的客观证据
   - 连通性任务（网络、WebRTC、数据传输等）必须提供真实连通成功的证据
   - 可接受的证据形式：
     - 端到端测试输出日志（包含成功标志）
     - 实际数据流截图/录屏（显示数据通而非仅信令通）
     - 抓包或调试日志（证明数据包实际传输）
   - 禁止仅凭代码审查或单元测试通过就关闭连通性任务
   - 证据必须附加到任务备注中，便于审计
