...
class WebRTCBlock implements Block {
  ...
  Future<Ack> _startLoopback(Command cmd) async {
    final sourceType = cmd.payload?['sourceType'] ?? 'screen';
    final sourceId = cmd.payload?['sourceId'];
    final cropRect = cmd.payload?['cropRect'];
    final fps = cmd.payload?['fps'] ?? 30;
    final width = (cmd.payload?['width'] as int?) ?? 1920;
    final height = (cmd.payload?['height'] as int?) ?? 1080;
    final bitrateKbps = (cmd.payload?['bitrateKbps'] as int?) ?? computeHighQualityBitrateKbps(width: width, height: height);

    final mediaConstraints = <String, dynamic>{
      'audio': false,
      'video': {
        'deviceId': {'exact': sourceId ?? 'screen:0'},
        'mandatory': {
          'frameRate': fps,
          'minWidth': width,
          'minHeight': height,
