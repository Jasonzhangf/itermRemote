...
@property(nonatomic, assign) BOOL cropAppliedInConfig;
@property(nonatomic, strong) CIContext *ciContext;
@property(nonatomic, copy) FlutterEventSink eventSink;
@property(nonatomic, copy) NSString *sourceId;
@property(nonatomic, assign) uint32_t windowId;
@property(nonatomic, copy) NSString *firstFrameDumpPath;
@end
...
- (instancetype)initWithCaptureDelegate:(id<RTCVideoCapturerDelegate>)captureDelegate {
  ...
  if (self) {
    ...
    _cropAppliedInConfig = NO;
    _windowId = 0;
    _firstFrameDumpPath = nil;
  }
  return self;
}
...
- (void)startWithWindowId:(uint32_t)windowId windowNameFallback:(NSString *)windowName fps:(NSInteger)fps cropRectNormalized:(NSDictionary *)cropRect {
  ...
  // Optional: dump first captured frame to a PNG file.
  // Used by host_test_app verification to produce evidence without screencapture.
  NSString *dump = [[[NSProcessInfo processInfo] environment] objectForKey:@"ITERMREMOTE_FIRST_FRAME_PATH"];
  if (dump && dump.length > 0) {
    self.firstFrameDumpPath = dump;
  } else {
    self.firstFrameDumpPath = nil;
  }
  self.hasCrop = NO;
  self.cropRectNorm = CGRectZero;
  self.cropAppliedInConfig = NO;
  ...
  [SCShareableContent getShareableContentWithCompletionHandler:^(SCShareableContent * _Nullable content,
    ...
    CGFloat cfgWidthPts = target.frame.size.width;
    CGFloat cfgHeightPts = target.frame.size.height;
    self.cropAppliedInConfig = NO;
    if (self.hasCrop) {
      ...
      CGFloat cropW = target.frame.size.width * nw;
      CGFloat cropH = target.frame.size.height * nh;
      CGFloat cropX = target.frame.size.width * nx;
      CGFloat cropY = target.frame.size.height * ny;
      if (cropW >= 1.0 && cropH >= 1.0) {
        // If supported, configure the stream to capture only the crop rect.
        if (@available(macOS 13.0, *)) {
          if ([config respondsToSelector:@selector(setSourceRect:)]) {
            config.sourceRect = CGRectMake(cropX, cropY, cropW, cropH);
            self.cropAppliedInConfig = YES;
            cfgWidthPts = cropW;
            cfgHeightPts = cropH;
            NSLog(@"[SCK] sourceRect applied: (%.1f,%.1f %.1fx%.1f)", cropX, cropY, cropW, cropH);
          }
        }
      }
    }
    config.width = (int)MAX(1, ceil(cfgWidthPts * scale));
    config.height = (int)MAX(1, ceil(cfgHeightPts * scale));
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
  ...
  // NOTE: Do not create a separate cropped CVPixelBuffer on macOS 26.x.
  // Releasing it while RTC is still reading can cause EXC_BAD_ACCESS.
  CVPixelBufferRef croppedPixelBuffer = nil;
  int x = 0;
  int yTop = 0;
  int w = 0;
  int h = 0;
  if (self.hasCrop && !self.cropAppliedInConfig) {
    if (srcW > 0 && srcH > 0) {
      CGFloat nx = MAX(0.0, MIN(1.0, self.cropRectNorm.origin.x));
      CGFloat ny = MAX(0.0, MIN(1.0, self.cropRectNorm.origin.y));
      CGFloat nw = MAX(0.0, MIN(1.0, self.cropRectNorm.size.width));
      ...
      x = (int)llround(nx * (double)srcW);
      yTop = (int)llround(ny * (double)srcH);
      w = (int)llround(nw * (double)srcW);
      h = (int)llround(nh * (double)srcH);
  ...
  // Keep `pixelBuffer` as the original sample buffer; crop metadata is carried
  // by `RTCCVPixelBuffer` below.

  // NOTE: Cropping path creates an RTCCVPixelBuffer that references the source
  // CVPixelBuffer. If we later release `croppedPixelBuffer` too early, the
  // RTC pipeline may still read from freed memory and crash.
  //
  // To keep this stable, never swap to a separate `croppedPixelBuffer` here;
  // instead, pass the RTCCVPixelBuffer with crop metadata directly.
  OSType fmt = CVPixelBufferGetPixelFormatType(originalPixelBuffer);
  ...
  RTCCVPixelBuffer *buffer = nil;
  if (self.hasCrop && !self.cropAppliedInConfig && w > 0 && h > 0) {
    // Provide crop metadata without allocating a new CVPixelBuffer.
    buffer = [[RTCCVPixelBuffer alloc] initWithPixelBuffer:originalPixelBuffer
                                              adaptedWidth:(int)srcW
                                             adaptedHeight:(int)srcH
                                                 cropWidth:w
                                                cropHeight:h
  ...
  if (self.eventSink) {
    ...
    if (outW > 0 && outH > 0 &&
      ...
      NSMutableDictionary *payload = [@{
        ...
        @"cropAppliedInConfig": @(self.cropAppliedInConfig),
      } mutableCopy];
      if (self.hasCrop) {
        payload[@"cropRect"] = @{
  ...
  if (!self.sentFirstFrame) {
    ...
    // Best-effort: dump the first frame to disk for deterministic verification.
    // This allows host_test_app to compare evidence with the actual capture pipeline.
    if (self.firstFrameDumpPath != nil) {
      @try {
        ...
        if (ciImage) {
          ...
          // Use the pixel buffer's full extent.
          CGRect rect = CGRectMake(0, 0, (CGFloat)srcW, (CGFloat)srcH);
          CGImageRef cg = [ctx createCGImage:ciImage fromRect:rect];
          if (cg) {
            NSBitmapImageRep *rep = [[NSBitmapImageRep alloc] initWithCGImage:cg];
            NSData *png = [rep representationUsingType:NSBitmapImageFileTypePNG properties:@{}];
            if (png) {
              [png writeToFile:self.firstFrameDumpPath atomically:YES];
              NSLog(@"[SCK] first frame dumped: %@", self.firstFrameDumpPath);
            }
            CGImageRelease(cg);
          }
        }
      } @catch (NSException *ex) {
        NSLog(@"[SCK] first frame dump failed: %@", ex);
      }
    ...
    if (deviceId != nil && [deviceId isKindOfClass:[NSDictionary class]]) {
      ...
      useDefaultScreen = YES;
	    ...
	    if (mandatory != nil && [mandatory isKindOfClass:[NSDictionary class]]) {
	      ...
	      id firstFrameAny = mandatory[@"itermremoteFirstFramePath"];
	      if (firstFrameAny != nil && [firstFrameAny isKindOfClass:[NSString class]]) {
	        firstFramePath = (NSString*)firstFrameAny;
	      }
	    }
	  }
	  RTCDesktopCapturer* desktopCapturer;
	  RTCDesktopSource* source = nil;
	  BOOL startedSckDirect = NO;
	  if (useDefaultScreen) {
	    desktopCapturer = [[RTCDesktopCapturer alloc] initWithDefaultScreen:self
	                                                        captureDelegate:videoSource];
	  } else {
	    // Allow callers to provide a raw CGWindowID directly as deviceId.exact.
	    // This bypasses RTCDesktopMediaList (and its enumeration/thumbnails).
	    if (@available(macOS 12.3, *)) {
	      NSCharacterSet* nonDigits = [[NSCharacterSet decimalDigitCharacterSet] invertedSet];
	      if (sourceId != nil && [sourceId rangeOfCharacterFromSet:nonDigits].location == NSNotFound) {
	        FlutterSCKInlineCapturer* sckDirect = [[FlutterSCKInlineCapturer alloc] initWithCaptureDelegate:videoSource];
	        sckDirect.eventSink = self.eventSink;
	        sckDirect.sourceId = sourceId ?: @"";
	        if (firstFramePath != nil) {
	          sckDirect.firstFrameDumpPath = firstFramePath;
	        }
	        uint32_t windowId = (uint32_t)[sourceId intValue];
	        [sckDirect startWithWindowId:windowId windowNameFallback:@"" fps:fps cropRectNormalized:cropRect];
	        NSLog(@"start desktop capture (SCK-direct): windowId=%u fps=%lu", windowId, fps);
	        self.videoCapturerStopHandlers[trackUUID] = ^(CompletionHandler handler) {
	          NSLog(@"stop desktop capture (SCK-direct): windowId=%u trackID %@", windowId, trackUUID);
	          [sckDirect stop];
	          handler();
	        };
	        desktopCapturer = nil;
	        source = nil;
	        startedSckDirect = YES;
	      }
	    }

	    if (!startedSckDirect && source == nil) {
	    ...
	    if (source.sourceType == RTCDesktopSourceTypeWindow) {
          ...
          sckCapturer.sourceId = sourceId ?: @"";
	        if (firstFramePath != nil) {
	          sckCapturer.firstFrameDumpPath = firstFramePath;
	        }
	        uint32_t windowId = (uint32_t)[sourceId intValue];
...
- (BOOL)buildDesktopSourcesListWithTypes:(NSArray*)types
  ...
  if (captureWindow) {
    ...
    // Avoid eager thumbnail generation on macOS 26.x where CIContext can crash.
    [_window UpdateSourceList:forceReload updateAllThumbnails:NO];
